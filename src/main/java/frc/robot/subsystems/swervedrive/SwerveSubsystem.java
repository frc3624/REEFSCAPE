...
{
  return run(() -> {
    Optional<PhotonPipelineResult> resultOpt = Cameras.CENTER_CAM.getLatestResult();
    
    if (resultOpt.isPresent() && resultOpt.get().hasTargets())
    {
      ...
      // Simple proportional control to turn toward target
      double rotationSpeed = -yawRadians * 2.0; // Adjust multiplier for sensitivity
      
      // Move forward while turning toward target
      double forwardSpeed = 1.0; // meters per second
      
      // Only move forward if we're reasonably aligned (within 15 degrees)
...
public Command driveToAprilTagDistance(double targetDistanceMeters)
{
  return run(() -> {
    ...
    if (resultOpt.isPresent() && resultOpt.get().hasTargets())
    {
      ...
      double currentDistance = camToTarget.getTranslation().getNorm();
      double yawDegrees = target.getYaw();
      double yawRadians = Math.toRadians(yawDegrees);
      ...
      double forwardSpeed = distanceError * 0.5; // Adjust multiplier as needed
